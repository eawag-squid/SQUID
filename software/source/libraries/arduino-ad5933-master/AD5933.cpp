 ...
 *
 * @return The temperature in celsius, or -1 if fail.
 */
 ...
 *
 * @param Voltage range to select. Use constants or 1/2/3/4.
 * @return Success or failure
 */
bool AD5933::setVoutRange(byte voutRangeNo) {
    // Get the current value of the control register
    byte val;
    if (!getByte(CTRL_REG1, &val))
        return false;

    // Clear out bits D9 and D10, which are the excitation voltage range select bits
    val &= 0xF9;

    // Determine what gain factor was selected
    if (voutRangeNo == VOUT_RANGE_1 || voutRangeNo == 1) {
        // Set output voltage range to 2Vpp in CTRL_REG1
        val |= VOUT_RANGE_1;
        return sendByte(CTRL_REG1, val);
    } else if (voutRangeNo == VOUT_RANGE_2 || voutRangeNo == 2) {
        // Set output voltage range to 1Vpp in CTRL_REG1
        val |= VOUT_RANGE_2;
        return sendByte(CTRL_REG1, val);
    } else if (voutRangeNo == VOUT_RANGE_3 || voutRangeNo == 3) {
        // Set output voltage range to 0.4Vpp in CTRL_REG1
        val |= VOUT_RANGE_3;
        return sendByte(CTRL_REG1, val);
    } else if (voutRangeNo == VOUT_RANGE_4 || voutRangeNo == 4) {
        // Set output voltage range to 0.2Vpp in CTRL_REG1
        val |= VOUT_RANGE_4;
        return sendByte(CTRL_REG1, val);
    } else {
        return false;
