/*! \zmodem_rz.cpp
 * \Utilities for zmodem protocol RZ commands implementation 
...
//#else
    ...
    fout = SD.open(Pathname, FILE_WRITE); //    if (!fout.open(Pathname, O_WRITE | O_CREAT | O_AT_END))
    if(!fout)
//#endif
      return ERROR;
    rxbytes = fout.size();
